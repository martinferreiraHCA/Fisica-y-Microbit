...
const content = [
  ...
  bullet([t('Programar la micro:bit con la extensión '), b('FisicaBit USB'), t(' (un solo bloque) o a mano para Bluetooth, entendiendo qué hace el código.')]),
  ...
  mkTable([2800, 3236, 3036],
    ...
    [
      ...
      ['Programación en MakeCode', 'Un solo bloque: el sitio abre el editor con la extensión FisicaBit USB ya cargada', 'Código a mano + extensión bluetooth + "No Pairing Required"'],
  ...
  body([t('La plataforma trae su propia extensión de MakeCode, '), b('FisicaBit USB'), t(', que reduce todo el programa a un solo bloque. Ya no hace falta escribir código a mano: el sitio abre el editor con todo cargado.')]),

  h2('4.1 · Abrí el editor desde el sitio'),
  step(1, [t('En la barra de herramientas presioná '), b('EXTRAS ▾'), t(' y elegí '), b('MAKECODE + FISICABIT'), t('.')]),
  pic('14_extras_makecode.png', 240, 336),
  cap('El editor se abre desde EXTRAS ▾ → MAKECODE + FISICABIT.'),
  step(2, [t('Se abre el '), b('editor oficial de MakeCode'), t(' (makecode.microbit.org, siempre en su versión vigente) dentro del sitio, con un proyecto nuevo que ya tiene cargada la extensión '), b('FisicaBit USB'), t(': los bloques '), b('“enviar a fisicabit.com”'), t(' aparecen sin buscar nada. Al volver a entrar, reabre tu última práctica.')]),
  infoBox([
    b('La barra flotante “FisicaBit”: '),
    t('sobre el editor hay una tira negra con '), b('Guardar en Drive'), t(' (sube la práctica a tu Google Drive, carpeta fisicabit / prácticas de micro:bit, y desde ahí se guarda sola), '), b('Mis prácticas ▾'), t(' (retomar prácticas guardadas o crear otra), '), b('Soltar USB'), t(' (libera el puerto si la grabación falla), '), b('Ayuda'), t(' y '), b('✕ Volver'), t(' (regresa al sitio sin cerrar el editor). Se puede arrastrar desde ⋮⋮ si tapa algo.'),
  ]),
  infoBox([
  ...
  body([t('En la categoría '), b('FisicaBit USB'), t(' está el bloque que hace todo el trabajo. El programa mínimo son dos bloques:')]),
  bullet([t('En '), b('al iniciar'), t(': '), b('mostrar ícono ✓'), t(' — para ver que el programa arrancó.')]),
  bullet([t('En '), b('para siempre'), t(': '), b('enviar a fisicabit.com tiempo y [leer sensor interno Acelerómetro X] cada [100] ms'), t('.')]),
  body([t('En JavaScript se ve así (MakeCode alterna entre bloques y texto sin perder nada):')]),
  ...codeWhy([
    ['basic.showIcon(IconNames.Yes)', 'feedback: el programa corre'],
    ['basic.forever(function () {', null],
    ['    FisicaBitSerial.enviar1(', 'el bloque "enviar a fisicabit.com"'],
    ['        FisicaBitSerial.leerSensorInterno(TipoSensorInterno.AcelerometroX),', 'qué sensor medir'],
    ['        100)', 'una muestra cada 100 ms (10 Hz)'],
    ['})', null],
  ]),
  body([t('Ese único bloque hace, por dentro, todo lo que antes había que programar a mano: toma el tiempo del micro:bit (desde 0), arma la línea '), code('tiempo,valor'), t(', la envía por el puerto serial y espera lo justo para mantener el ritmo de muestreo. Por eso el timestamp viene incluido siempre.')]),
  bullet([b('¿Más de una variable? '), t('El mismo bloque tiene variantes de 2, 3 y 4 valores ('), b('enviar a fisicabit.com tiempo, … y … cada … ms'), t('). En la plataforma, ajustá NÚMERO DE VARIABLES para que coincida.')]),
  bullet([b('¿Sin tiempo? '), t('El bloque '), b('enviar a fisicabit.com sin tiempo'), t(' manda solo los valores (para eventos, botones o tu propia pausa). En ese caso, desactivá la casilla “Micro:bit envía timestamp”.')]),
  bullet([b('Sensores internos: '), t('temperatura, acelerómetro X/Y/Z, nivel de luz, brújula, nivel de sonido (v2) y fuerza G, con el bloque '), b('leer sensor interno'), t('.')]),
  bullet([b('Sensores externos: '), b('leer sensor analógico'), t(' en P0/P1/P2 (potenciómetro, LDR: 0 a 1023), '), b('leer sensor digital'), t(' en P8 (PIR, interruptor: 0/1), '), b('temperatura NTC 10K'), t(' y '), b('HC-SR04 distancia'), t(' (ultrasónico, en cm). Conexiones típicas: señal → pin, VCC → 3V, GND → GND.')]),

  h2('4.3 · Velocidad real: “para siempre” vs “bucle rápido”'),
  body([t('El límite de velocidad no lo pone el puerto (115200 baudios'), fn(4), t(' ≈ 11.500 caracteres/segundo) sino el '), b('scheduler de MakeCode'), fn(5), t(': cada vuelta de '), b('para siempre'), t(' agrega ~20 ms ocultos. Hasta ~10–20 Hz'), fn(6), t(' eso no molesta. Para fenómenos rápidos (caída libre, choques), la extensión trae el bloque '), b('bucle rápido para fisicabit.com cada [20] ms'), t(', que reemplaza a '), b('para siempre'), t(' y mantiene el ritmo con precisión — el bloque de envío va adentro:')]),
  ...codeWhy([
    ['FisicaBitSerial.bucleMuestreo(20, function () {', '"bucle rápido cada 20 ms": ~50 Hz reales'],
    ['    FisicaBitSerial.enviar1(', null],
    ['        FisicaBitSerial.leerSensorInterno(TipoSensorInterno.AcelerometroX), 20)', null],
    ['})', null],
  ]),
  mkTable([3600, 2400, 3072],
    ['Bloques', 'Frecuencia real', 'Uso típico'],
    [
      ['para siempre + enviar cada 500–1000 ms', '1–2 Hz', 'Temperatura, enfriamiento'],
      ['para siempre + enviar cada 100 ms', '~10 Hz', 'Luz, procesos lentos, uso general'],
      ['para siempre + enviar cada 50 ms', '~15–20 Hz', 'Movimiento general'],
      ['bucle rápido cada 20 ms', '~50 Hz', 'Oscilaciones, choques'],
      ['bucle rápido cada 10 ms', '~100 Hz', 'Máximo recomendado'],
    ]),
  spacer(),

  h2('4.4 · Grabar el programa'),
  step(3, [t('Conectá la micro:bit por USB y presioná '), b('Descargar'), t(' en el editor. Si el navegador ofrece '), b('emparejar el dispositivo (WebUSB)'), fn(7), t(', aceptá: se graba solo. Si no, copiá el archivo '), code('.hex'), fn(8), t(' a la unidad '), b('MICROBIT'), t(' que aparece como un pendrive.')]),
  step(4, [t('Esperá a que la luz naranja del reverso deje de parpadear y aparezca el ✓ en los LED: el programa ya está corriendo.')]),
  ...
  infoBox([
    b('Si la grabación directa falla: '),
    t('suele ser porque la plataforma tiene el puerto USB abierto. Presioná '), b('Soltar USB'), t(' en la barra flotante de FisicaBit y reintentá '), b('Descargar'), t('.'),
  ]),

  h2('4.5 · Conectar desde la plataforma'),
  step(5, [t('Presioná '), b('✕ Volver'), t(' para regresar al sitio (el editor queda abierto por si querés retocar el programa).')]),
  step(6, [t('Presioná '), b('CONECTAR'), t(' en la barra de herramientas. En la ventanita del navegador elegí el puerto de la micro:bit ('), b('“BBC micro:bit CMSIS-DAP”'), t(' o similar) y presioná '), b('Conectar'), t('. La velocidad (115200 baudios) la configura la plataforma sola. El cable debe ser '), b('USB de datos'), t(' (algunos solo cargan).')]),
  ...howBox('LA COMUNICACIÓN SERIAL POR USB', [
    t('La micro:bit convierte cada línea de texto en una secuencia de bytes que viaja por el cable. Del lado de la computadora, el navegador la recibe mediante Web Serial y la entrega a la plataforma, que corta el flujo en líneas (por el salto de línea) y en columnas (por las comas). Es el mismo mecanismo de los viejos puertos serie de PC: simple, robusto y con latencia baja y pareja.'),
  ]),
  ...
  body([t('La extensión FisicaBit USB es solo para el cable; por Bluetooth el programa se escribe a mano, con el método clásico. La conexión inalámbrica usa el servicio '), b('UART'), fn(9), t(' sobre '), b('Bluetooth Low Energy'), t('. Requiere dos configuraciones en MakeCode que, cuando faltan, son la causa típica de que “no conecte”.')]),
  ...
  mkTable([2600, 6472],
    ...
    [
      ...
      ['bucle rápido (FisicaBit)', 'Bloque de la extensión FisicaBit USB que reemplaza a "para siempre" para muestrear a 50–100 Hz con ritmo preciso, sin el sobrecosto oculto de forever. El bloque de envío va adentro.'],
      ...
      ['MAKECODE + FISICABIT', 'Opción del menú EXTRAS ▾ que abre el editor oficial de MakeCode dentro del sitio, con la extensión FisicaBit USB ya cargada, y guarda las prácticas en el navegador o en Google Drive.'],
